...
var WeekViewComponent = /** @class */ (function () {
    ...
    WeekViewComponent.prototype.onDataLoaded = function () {
        var eventSource = this.eventSource, len = eventSource ? eventSource.length : 0, startTime = this.range.startTime, endTime = this.range.endTime, currentViewIndex = this.currentViewIndex, rooms = this.views[currentViewIndex].rooms, normalEventInRange = false, rangeStartRowIndex = this.startHour * this.hourSegments, rangeEndRowIndex = this.endHour * this.hourSegments, allRows = 24 * this.hourSegments;
        for (var day = 0; day < 7; day += 1) {
            for (var roomIdx = 0; roomIdx < rooms.length; roomIdx += 1) {
        ...
        for (var i = 0; i < len; i += 1) {
            ...
            if (eventEndTime < startTime || eventStartTime > endTime) {
                ...
                var weekDayStartIndex = event_1.startDate.getDay();
